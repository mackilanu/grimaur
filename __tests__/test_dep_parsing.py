...
class NormalizeDepTests(unittest.TestCase):
	def test_plain(self):
		self.assertEqual(grimaur._normalize_dep("python"), "python")

	def test_preserves_dashes(self):
		self.assertEqual(grimaur._normalize_dep("lib32-glibc"), "lib32-glibc")

	def test_strips_each_version_operator(self):
		cases = [
			("python>=3.10", "python"),
			("python<=3.12", "python"),
			("python>3", "python"),
		...
		for raw, want in cases:
			with self.subTest(raw=raw):
				self.assertEqual(grimaur._normalize_dep(raw), want)

	def test_strips_repo_qualifier(self):
		self.assertEqual(grimaur._normalize_dep("extra:python"), "python")
		self.assertEqual(grimaur._normalize_dep("extra:python>=3.10"), "python")

	def test_strips_outer_whitespace(self):
		self.assertEqual(grimaur._normalize_dep("  python  "), "python")

	def test_empty_in_empty_out(self):
		self.assertEqual(grimaur._normalize_dep(""), "")
		self.assertEqual(grimaur._normalize_dep("   "), "")

	def test_compound_operators(self):
		# Multiple version operators collapse to just the name.
		self.assertEqual(grimaur._normalize_dep("python>=3.10<4"), "python")
...
class ParseDependenciesTests(unittest.TestCase):
	def test_returns_pkgbase_and_desc(self):
		pkgbase, desc, _deps = grimaur.parse_dependencies(SAMPLE_SRCINFO)
		self.assertEqual(pkgbase, "examplepkg")
		self.assertEqual(desc, "An example package for tests")

	def test_depends_are_normalized(self):
		_pkgbase, _desc, deps = grimaur.parse_dependencies(SAMPLE_SRCINFO)
		self.assertEqual(
			deps.depends,
			{"python", "requests", "whitespace-pkg"},
		)

	def test_makedepends_and_checkdepends_normalized(self):
		_pkgbase, _desc, deps = grimaur.parse_dependencies(SAMPLE_SRCINFO)
		self.assertEqual(deps.makedepends, {"git", "python-build"})
		self.assertEqual(deps.checkdepends, {"python-pytest"})

	def test_optdepends_keep_descriptions_verbatim(self):
	...
	def test_missing_pkgbase_raises(self):
		with self.assertRaises(grimaur.AurGitError):
			grimaur.parse_dependencies("pkgname = nope\ndepends = python\n")

	def test_comments_and_blank_lines_ignored(self):
		srcinfo = "pkgbase = x\n\n# comment\n\tdepends = foo\n"
		_pkgbase, _desc, deps = grimaur.parse_dependencies(srcinfo)
