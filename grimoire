...
class UpdateCandidate:
	...
	local_head: 		str | None
	# Clone HEAD before the update probe advances it -- the installed version's commit,
	# diffed against the new one in the build review. None when there was no prior clone.
	prev_head: 		str | None = None


@dataclass(frozen=True, slots=True)
class UpdateContext:
	# What an update rebuild hands install_package beyond a fresh install: the version
	# label for the "Updating X (a -> b)" line, and the pre-update commit the PKGBUILD
	# review diffs against. Absent (None) on a plain install.
	target: 	str
	prev_head: 	str | None = None
...
def _page_file(path: Path) -> None:
	# Show a file through $PAGER (default `less`), streams inheriting the tty so it stays
	# scrollable -- unlike run_command's captured output. Falls back to a plain dump when
	# no pager is on PATH or it can't launch.
	argv = shlex.split(os.environ.get("PAGER") or "less")
	if argv and shutil.which(argv[0]):
		with contextlib.suppress(OSError):
			subprocess.run([*argv, str(path)], check=False)  # noqa: S603 - pager from env, path internal
			return
	print(path.read_text())


def _review_pkgbuild(
	package_dir: Path, package: str, diff_base: str | None = None
) -> None:
	# Safety gate before makepkg runs the PKGBUILD's code. On an update we have the
	# previously-built commit (diff_base), so show just what changed since the installed
	# version (git diff, like paru/yay); on a fresh install show the whole PKGBUILD.
	# Declining builds as before; reviewing still lets the user abort. No-op off a tty,
	# where prompt_confirm returns False.
	pkgbuild = package_dir / "PKGBUILD"
	if not pkgbuild.is_file():
		return
	head = _local_head(package_dir)
	show_diff = diff_base is not None and head is not None and diff_base != head
	noun = "changes to" if show_diff else "PKGBUILD for"
	if not prompt_confirm(
		style(f"Review {noun} {package} before building? [y/N]: ", YELLOW)
	):
		return
	if show_diff:
		# capture=False inherits the tty, so git pages the diff itself (GIT_PAGER/
		# core.pager/$PAGER, default less); check=False ignores a pager-quit exit code.
		run_command(
			["git", "-C", str(package_dir), "diff", f"{diff_base}..{head}"],
			check=False,
		)
	else:
		_page_file(pkgbuild)
	if not prompt_confirm(style(f"Proceed with building {package}? [y/N]: ", YELLOW)):
		raise GrimoireErr("Installation aborted by user")
...
def install_package(
	...
	*,
	...
	if is_installed(package):
		...
		if update:
			print(f"Updating {style(package, BOLD)} ({installed_ver} -> {update.target})")
		else:
			print(
	...
	# Review gate for AUR PKGBUILDs (an origin-less / aur.archlinux.org clone). A custom
	# repo registered by the user is treated as trusted and skips straight to the build.
	if not noconfirm and _origin_label(package_dir) == "aur":
		_review_pkgbuild(
	...
	*,
	...
	for package, installed_version in candidates:
		...
		if skip_devel and is_vcs_package(package):
			continue
		# Read the clone's HEAD before the probe fetches/resets it, so the build review
		# can diff the installed version against the update.
		prev_head = _local_head(dest_root / package)
		try:
		...
		try:
			install_package(
				...
				update=UpdateContext(
					_update_target_label(candidate), candidate.prev_head
				),
			)
		except GrimoireErr as exc:
